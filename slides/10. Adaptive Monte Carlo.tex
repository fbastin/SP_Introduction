...
\[
\textcolor{red}{\min_{x \in S} g(x) = \EE_P \left[ G(x, \bxi) \right]},
\]
where
\begin{itemize}
	\item
...
$C(S)$ is a Banach space, i.e. a normed vector space, complete under the distance induced by its norm.
A metric space $M$ is said complete or complete space if every Cauchy sequence in $M$ has a limit in $M$ (i.e. it converges in $M$).
...
$\exists$ $K(\xi) \geq 0$ such that $\EE[K(\xi)]$ is finite, and
$\forall\, x_1$, $x_2 \in S$ and a.e. $\xi$,
\[
| G(x_1, \xi) - G(x_2, \xi) )| \leq K(\xi) \| x_1 - x_2\|,
\]
We assume moreover that $\EE[K^2(\xi)] < \infty$.% is finite.
\end{enumerate}
...
\[
\hat{v}_N = \min_{x \in S} \hat{g}_N(x) \mbox{ and } v^*= \min_{x \in S} g(x),
\]
then
\[
...
Under some additional conditions, we also have the convergence of $\EE[\hat{v}_N]$ to $v^*$.
...
%Alternatively,
We can start with a small sample and extend it over the iterations:
...
An external approach is known as \textcolor{red}{retrospective approximation} consists in repeatedly applying the optimization algorithm with samples of increasing sizes.
...
\item
$G(\cdot,\xi)$ is Lipschitz with Lipschitz constant $L(\xi)$ on $S$ a.s., and $\EE[L(\xi)] < \infty$.
\item
$G(\cdot,\xi)$ is continuously differentiable at any $x \in \cB(x^*, \epsilon)$, $\epsilon > 0$, a.s.
\item
$\exists\, x \in S$ such that $\EE[\|\nabla_x G(x,\xi)\|^2] < \infty$.
\item
...
Let $\alpha_{\delta}$ be the quantile of a $\cN(0,1)$ associated to some significance level $\delta$, i.e.
$$
P_{\bxi} [ -\alpha_{\delta} \leq Y \leq \alpha_{\delta} ] = \delta,
$$
where $Y \sim \cN(0,1)$.
\item
Central limit theorem (background notes, where the quantile is written $\Phi^{-1}(1-\delta/2)$):
\[
g(x) - \hat{g}_N(x) \Rightarrow \cN \left( 0, \frac{\sigma^2(x)}{N} \right),
\]
%where $\sigma^2(x)$ is the variance of $g$, taken at the point $x$,
\item
...
\item
If the adequacy of the model w.r.t. the accuracy of the objective function is poor, we can increase the sample size in an attempt to correct this deficiency.
\item
...
\item[Step 4. Comparison of decreases]
...
\begin{itemize}
\item 
If $\tau_1^k \geq \tau_2^k$, 
%the ratio between the current sample size and the potential next one is smaller than the ratio between $\Delta m_k$ and the estimated error.
%If the sample size increases, the error decreases for a similar $\Delta m_j^{N_j}$ ($j \geq k$), and therefore $\tau_1^k$ increases.
%We capitalize on $\tau_1^k$ by
compute a sample size smaller than  $N^s$, such that an improvement of the order $\epsilon_\delta^{N_k}(z_k)$ would be reached in approximately $\lceil \tau_1^k \rceil$ iterations if $\tau_1^j$ is similar to $\tau_1^k$ for $j$ close to $k$.
\item
If $\tau_1^k < \tau_2^k$, it can nevertheless be cheaper to continue to work with a smaller sample size, defined again as $\lceil \chi_1 N_{\max} \rceil$, as long as $\tau_1^k$ is greater than some threshold $\nu_1$ (for instance 0.2).
...
This can lead to $\rho_k < \eta_1$, even if the model $m_k^{N_k}$ gives a good prediction for the sample size $N^k$.
...
\[
\rho_k = \frac{\hat{g}_{N^-}(x_k) - \hat{g}_{N^-}(x_k+s_k)}{\Delta
m_k^{N^-}}.
\]
...
\]
for all $k$ large enough, then, a.s., the algorithm converges 
with $N_i = N_{\max}$ for all $i$ large enough.
%in a finite number of iterations with a final sample size equal to $N_{\max}$, or the number of iterations is infinite and there exists some $j$ such that for all the iterations $i$, $i \geq j$, $N_i$ is equal to $$.
\end{theo}
...
\begin{frame}[fragile]
\frametitle{Application to discrete choice theory}
				...
				\begin{center}
					\scalebox{0.8}{
						\begin{tikzpicture}[>=latex, every node/.style={font=\small}]
							...
							\node (car)  at (1.8, 1.2) {\textbf{car}};
							\node (train) at (1.8, 0.6) {\textbf{train}};
							\node (bus)  at (1.8, 0.0) {\textbf{bus}};
							\node (bike) at (1.8,-0.6) {\textbf{bike}};
							\node (walk) at (1.8,-1.2) {\textbf{walk}};
							\draw[->] (i.east) -- (car.west);
							\draw[->] (i.east) -- (train.west);
							\draw[->] (i.east) -- (bus.west);
							\draw[->] (i.east) -- (bike.west);
							\draw[->] (i.east) -- (walk.west);
						\end{tikzpicture}
					}%
				\end{center}%
			\end{minipage}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	...
	\begin{frame}{Mixed Logit models}
		...
		\begin{footnotesize}
			...
			\end{center}
			$\boldsymbol{\gamma}$: {\blue random vector}, e.g. vector of independent
			$N(0,1)$);\\
...
\begin{frame}{Error estimation}
		...
		\begin{footnotesize}
			...
			\end{center}
			$\delta$: significance level; $\alpha_{0.9} \approx 1.65$.
			
				Bias of simulation (Taylor expansion):
				\begin{center}
					\colorbox{orange}{
						$B := \EE[SLL^R(\theta)] - LL(\theta) =
...
\begin{frame}{Algorithms comparison}
		...
		\begin{footnotesize}
			...
			\begin{center}
				\hspace*{\stretch{1}}
				\includegraphics[width=0.23\linewidth]{2dims.eps}
				\includegraphics[width=0.23\linewidth]{5dims.eps}
				\includegraphics[width=0.23\linewidth]{10dims.eps}
				\hspace*{\stretch{1}}
				
				\includegraphics[width=0.25\linewidth]{legend.eps}
			\end{center}
...
\begin{tiny}
{%\red
\hspace*{5.5cm}$\downarrow$\hspace*{2.6cm}$\downarrow$\\
\hspace*{4.8cm}Starting value\hspace*{1.3cm}Maximum value\\
}
\end{tiny}
...
\begin{frame}{Possible improvement: how to draw samples?}
	...
	\begin{itemize}
		...
		\item Incremental \textit{Common Random Numbers} (CRN): take the first $N_K$ numbers from a predefined sequence of random draws: $\mathcal{N}_k = \{ \xi_1, \ldots, \xi_{N_k} \}$
	\end{itemize}
...
\begin{frame}{Incremental Common Random Numbers}    
	\centering
	\includegraphics[scale=0.43]{DistTo_sampleSize_CRN.PNG}
\end{frame}

\begin{frame}{IRN/CRN combination (ICRN)}
	...
	\begin{enumerate}
		...
		\item $N_{k+1} < N_k$
		$$\mathcal{N}_{k+1} = \mathcal{N}_k \backslash  \{\text{uniformly randomly selected elements in } \mathcal{N}_k\}$$
	\end{enumerate}
