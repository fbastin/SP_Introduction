...
{\red \forall s \in S} \\
\sum_{i \in N} \omega_{iT{\red s}}x_{iT{\red s}} - y_{\red s} + w_{\red s} & = G,
...
Equivalently,
\[
\left( \sum_{s' \in S_s^t} p_{s'} \right) x_{its} = \sum_{s' \in S_s^t} p_{s'} x_{its'} \quad \forall\ i,t,s,
\]
or
\begin{align*}
 x_{its}
&= \frac{\sum_{s' \in S_s^t} p_{s'} x_{its'}}{\sum_{s' \in S_s^t} p_{s'}} \quad \forall\ i,t,s \\
&= \EE[ x_{its'} | s' \in S_s^t ] \quad \forall\ i,t,s.
\end{align*}
