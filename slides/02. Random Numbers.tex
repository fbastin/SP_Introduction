...
\begin{center}
	\href{https://xkcd.com/221/}{\includegraphics[scale=0.75]{random_number.png}}
\begin{small}
...
\end{frame}
	...
	\begin{frame}
		...
		Full period: $m$ if $c \ne 0$; if $c = 0$, then 0 is a fixed point of the
		recurrence and the best possible period is $m-1$, reached only when $m$ is
		prime (for $m = 2^k$ it drops to $m/4$).
		Consider the case $c \ne 0$.
...
function getlcg(seed::Integer, a::Integer,
    ...
    return function lcgrand()
        ...
        # in (0,1) when c = 0 and m is prime
        return state[]*invm
    end
end
...
stdmin = getlcg(1234, 16807, 0, 2^31-1)
	...
	\begin{frame}
		...
		\begin{center}
			...
			The $5000$ successive pairs $(u_{2i-1}, u_{2i})$ built from $10\,000$
			generated numbers.
		\end{center}
...
%		This kind of generators is very popular.
	...
	\begin{frame}
		...
		amongst the most popular and efficient generators (available in R as
		\texttt{L'Ecuyer-CMRG}, the default of the \texttt{parallel} package, and as
		MATLAB's \texttt{mrg32k3a}), although newer families like xoshiro or CBRNGs are
		often preferred today for raw performance.
		
		\medskip
		
		It combines 2 MRG's, both with $k=3$:
		\[
		\begin{array}{ll}
		m_1 = 2^{32}-209, & a_{11} = 0,\quad a_{12} = 1403580,\quad a_{13} = -810728,\\[2pt]
		m_2 = 2^{32}-22853, & a_{21} = 527612,\quad a_{22} = 0,\quad a_{23} = -1370589.
		\end{array}
		\]
	...
	\begin{frame}
		\frametitle{MRG32k3a: the equivalent MRG}
		
		The combination behaves like a single MRG of order $k = 3$, modulo
		$m = m_1 m_2$. Its coefficients follow from the Chinese remainder theorem
		($a_j \equiv a_{1,j} \bmod m_1$ and $a_j \equiv a_{2,j} \bmod m_2$):
		\[
		\begin{array}{rl}
		m   &= 18446645023178547541,\\[2pt]
		a_1 &= 18169668471252892557,\\[2pt]
		a_2 &= 3186860506199273833,\\[2pt]
...
\begin{frame}
	...
	\begin{frame}
		...
		$111101011001000$ is cut into consecutive blocks
		\[
		...
		{\red But} only $5$ distinct values appear: here $\gcd(\nu, 2^k-1) = \gcd(3,15) = 3$,
		so the output period collapses to $15/3 = 5$ --- see the next slide.
...
u2 = rand(rng)
	...
	\begin{frame}
		...
		\[
		X = F^{-1}(U) = \inf \lbrace x : F (x)  \geq U \rbrace \quad \textrm{\textcolor{red}{(Generalized inverse)}},
		\]
		the quantile function of the probability background notes.
		Then,
		\[
	...
	\begin{frame}
		...
		\begin{itemize}
		    ...
		    \vspace{0.3cm}
		    \item \textbf{Variance Reduction:} This monotonicity is what makes variance reduction techniques like \textit{Common Random Numbers} (CRN) or \textit{Antithetic Variates} reliable: it guarantees the sign of the induced correlation. Methods consuming a random number of uniforms, such as acceptance-rejection, lose that guarantee and may even destroy the synchronization between runs.
		    \vspace{0.3cm}
...
\theta &= 2 \pi U_2
\end{align*}
(in an implementation, make sure that $U_1 \in (0,1]$: most libraries return
uniforms in $[0,1)$, where $U_1 = 0$ would give $R = \infty$.)
\item
Set
\begin{align*}
X &= R \cos(\theta) \\
...
\begin{frame}
\frametitle{What \texttt{randn()} really does: the ziggurat}

Neither inversion nor Box-Muller: most libraries (Julia, R, NumPy) draw normal variates with
the {\blue ziggurat} method (Marsaglia and Tsang, 2000).

\vspace{0.2cm}

{\red Idea}: cover the density with 256 layers of \emph{equal area}, precomputed once in three
tables. Draw a single 52-bit word and cut it into pieces:
\begin{itemize}
\item 1 bit for the sign, 8 bits for the index of the layer, the rest for the position inside it;
\item if the point falls in the rectangular part of the layer, accept it --- one table lookup, one
multiplication, and \emph{no} transcendental function.
\end{itemize}

The fast path is taken in $98.5\%$ of the draws: in a tight loop, a normal variate costs barely
more than the uniform it consumes, some five times less than inversion.

\vspace{0.2cm}

{\red Price}: acceptance-rejection again ($1.02$ uniforms per variate, at random), hence neither
monotone nor synchronized --- no CRN, no antithetic variates, no QMC. And, unlike inversion, it
reads the \emph{individual bits} of the generator.

\end{frame}

\begin{frame}[fragile]
\frametitle{When the bits are not what the algorithm assumes}

The ziggurat asks for 52 \emph{independent} bits: the generator's own when it is
\texttt{UInt64}-native, and otherwise the \emph{mantissa} of a float:

\begin{footnotesize}
\begin{semiverbatim}
_rand52(r, ::Type\{Float64\}) = reinterpret(UInt64, rand(r, CloseOpen12()))
\end{semiverbatim}
\end{footnotesize}

\vspace{-0.1cm}
MRG32k3a is \texttt{Float64}-native and returns a multiple of $1/m_1$, $m_1 = 2^{32}-209$: some 32
random bits, stretched over a 52-bit mantissa. On $10^6$ draws (\texttt{RandomDataStreams} v0.2.0):

\begin{center}
\begin{footnotesize}
\begin{tabular}{lcc}
\hline
 & MRG32k3a stream & Xoshiro256++ stream \\
\hline
$P(\mbox{lowest bit} = 1)$ & {\red $0.334$} & $0.500$ \\
\texttt{randn}: mean / std & {\red $+0.340$ / $0.940$} & $+0.000$ / $1.000$ \\
\hline
\end{tabular}
\end{footnotesize}
\end{center}

That bit is the \emph{sign}: magnitudes are right, two draws out of three are positive.
\texttt{rand(rng, Normal())} and \texttt{MvNormal} inherit it, inversion does not, and SmallCrush
cannot see it --- it tests the uniforms, which are sound.

{\blue Rule of thumb}: draw normals from a \texttt{UInt64}-native stream (xoshiro, PCG, Philox), or
by inversion --- a bit-level method is a tacit contract with the generator, unenforced.
...
$$
	...
	= \Phi_{R} \left( \Phi^{-1} \left( F_1(x_1) \right),\ldots, \Phi^{-1}\left( F_d(x_d) \right) \right),
$$
where
\begin{itemize}
	\item $\Phi$ is the cdf of a $\calN(0,1)$,
	\item $\Phi_{R}$ is the cdf of a $d$-dimensional $\calN(\bszero,R)$.
\end{itemize}

Here $R$ must be a {\blue correlation} matrix (unit diagonal), not an arbitrary
covariance: otherwise the margins of $\Phi_{R}$ are not $\calN(0,1)$ and $C$ is
not a copula.
...
\begin{itemize}
    ...
    \item \textbf{Ziggurat:} Marsaglia, G. \& Tsang, W. W. (2000). \textit{The Ziggurat Method for Generating Random Variables}. \url{https://doi.org/10.18637/jss.v005.i08}
