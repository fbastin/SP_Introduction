...
\begin{frame}
		...
		Let us introduce the Galois field $\mathcal{F}_2$: it is simply the set $\lbrace 0,1 \rbrace$
...
\begin{itemize}
\item
Melissa O'Neil introduced the PCG family (\url{https://www.pcg-random.org/}), which applies permutation functions to the output of an LCG.
\item
Vigna (author of xorshift/xoshiro) strongly criticizes PCG (\url{https://pcg.di.unimi.it/pcg.php}). He argues that PCG's mechanism for generating independent streams (varying the LCG increment) lacks the rigorous guarantees of MRGs or CBRNGs.
\item
...
\item The most popular implementation, Philox, detailed next, is the default generator in TensorFlow and PyTorch (JAX uses Threefry, another CBRNG).
\end{itemize}
...
\begin{itemize}
	...
	\item NumPy \texttt{Generator}: PCG64 by default (Philox available); JAX: Threefry.
