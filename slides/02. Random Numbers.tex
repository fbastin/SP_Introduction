...
\begin{frame}
    ...
    \begin{itemize}
        \item \textbf{Understand the foundation:} How computers generate pseudo-random numbers uniformly distributed on $(0,1)$.
        \item \textbf{Explore generator architectures:} From classic Linear Congruential Generators (LCG) to modern Counter-Based RNGs (CBRNG).
        \item \textbf{Transform uniforms into target distributions:} Master the fundamental techniques (Inversion, Acceptance/Rejection) for drawing from arbitrary distributions.
        \item \textbf{Address multivariate settings:} Introduce the concepts of Cholesky decomposition and Copulae to model dependent random variables.
    \end{itemize}
\end{frame}
\begin{frame}
	%\frametitle{Outline}
...
\begin{frame}
		...
		on {\blue bits}, with {\blue linear recurrences in}\, $\mathcal{F}_2$.
		
		\mbox{}
		
		Recall the Galois field $\mathcal{F}_2$: the set $\lbrace 0,1 \rbrace$
		with addition and multiplication computed modulo 2.
		
		\mbox{}
		
		All operations are then {\blue bitwise}: shifts, xors and masks,
		which makes these generators {\blue numerically very fast}.
		
	\end{frame}
	
	\begin{frame}
		\frametitle{Generators on $\mathcal{F}_2$: state and output}
	...
	\begin{frame}
		...
		\begin{minipage}{0.57\linewidth}
			...
			In $\mathcal{F}_2$, addition is the {\blue exclusive-or} (xor):
	...
	\begin{frame}
		\frametitle{LFSR: a tiny example ($k = 4$, taps $r = 1$ and $r = 4$)}
	...
	\begin{frame}
		...
		The recurrence is a linear recurrence over $\mathcal{F}_2$ on the state vector
		(we assume $a_k \ne 0$):
		\[
		...
		\begin{itemize}
			...
			\item
			For {\blue Tausworthe} generators, $B = I$ (no output transformation).
		\end{itemize}
	...
	\begin{frame}
		...
		\]
		which, in $\mathcal{F}_2$, brings us back to the xor recurrence
		$x_n = x_{n-r} \oplus x_{n-k}$ of the LFSR.
		
		\vfill
		
		{\red Caveat.} Linearity in $\mathcal{F}_2$ is a strong structural flaw: plain
...
\item
However, Vigna (the author of xorshift generators) has a lot of criticisms regarding PCG: \url{https://pcg.di.unimi.it/pcg.php}.
\item
...
\item The most popular implementation, Philox, detailed next, is the default generator in TensorFlow, PyTorch, NumPy, and JAX.
\end{itemize}
...
\begin{itemize}
\item
{\blue Default variant} (PyTorch, TensorFlow, NumPy \texttt{Generator}, JAX,
cuRAND):
\[
...
\begin{tikzpicture}[>=latex, scale=0.92, every node/.style={font=\small}]
	...
	\draw[->] (M0.east) to[out=-25,in=155] node[pos=0.5, above]{$\mathrm{hi}_0$} (c1.west);
	\draw[->] (M0.south east) to[out=-60,in=170] node[pos=0.65, below]{$\mathrm{lo}_0$} (y3.west);
	\draw[->] (M1.east) to[out=25,in=205] node[pos=0.5, below]{$\mathrm{hi}_1$} (c0.west);
	\draw[->] (M1.south east) to[out=-35,in=180] node[pos=0.65, below]{$\mathrm{lo}_1$} (y1.west);
	\draw[->] (k0n) -- (c0.north);
	\draw[->] (k1n) -- (c1.south);
...
\end{align*}
where $(\mathrm{hi}_j, \mathrm{lo}_j)$ are the high/low 32-bit halves of $M_j x_j$.
...
\begin{itemize}
\item The key identifies an independent stream, and the counter its position: jumping ahead amounts to integer addition.
\item Stateless: no internal array to store or synchronize; the value at position $i$ does not depend on the draw order. Well-suited to GPUs and parallel Monte Carlo.
\item Reproducibility: identical (counter, key) yields identical bits on all architectures.
\item Natural fit for stochastic optimization: one stream per scenario or sample index.
\end{itemize}

{\red Adoption:}
\begin{itemize}
	\item NVIDIA cuRAND and PyTorch (GPU): Philox4$\times$32--10;
	\item TensorFlow: Philox by default, Threefry optional;
	\item NumPy \texttt{Generator}: Philox bit generator; JAX: Threefry.
	\item Modern ML leans on it: dropout masks and distributed training draw thread- or rank-independent numbers.
\end{itemize}
...
\end{frame}
	...
	This package is now officially registered in Julia. You can add it via:
	\begin{verbatim}
] add RandomDataStreams
	\end{verbatim}

	\mbox{}

	This package proposes an implementation of MRG32k3a with streams and substreams, following the same philosophy as counter-based generators: explicitly indexed, reproducible subsequences rather than synchronized states.

	\mbox{}

	Illustration (one independent stream per scenario, one substream per replication):
\begin{semiverbatim}
using RandomDataStreams
gen = MRG32k3aGen()
rng = next_stream!(gen) # Independent stream for a scenario

# For each replication:
next_substream!(rng)    # Move to the next substream
u = rand(rng)           # Draw random numbers
\end{semiverbatim}
...
\begin{itemize}
    \item \textbf{Why use inversion?} It guarantees a monotonic mapping from the uniform $U$ to the target $X$.
    \item This monotonicity is strictly required by variance reduction techniques like \textit{Common Random Numbers} (CRN) or \textit{Antithetic Variates}.
\end{itemize}

\mbox{}

		
		Preferred method when applicable: compatible with variance reduction and copulae.
		...
		\begin{itemize}
			...
			\item
			The principle still works for mixed distributions.
		\end{itemize}
	...
	\begin{frame}
		...
		\begin{minipage}{0.52\linewidth}
			...
			\begin{enumerate}
				...
				\item
				generate $U: U (0, 1)$ independently of $Y$;
				\item
				until $U \leq f (Y )/t(Y )$;
				\item
				return Y.
