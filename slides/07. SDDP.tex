...
Based on Anthony Papavasiliou's slides \url{https://ap-rg.eu/wp-content/uploads/2020/05/SDDP.pdf}.

\mbox{}

Introduced by Pereira and Pinto (1991), 1991.
...
\item
Complete recourse (no feasibility cuts needed).
\end{itemize}
...
Terminates if $z_{LB} \in (\overline{z}_K - \Phi^{-1}(1-\alpha/2) \hat{s}/\sqrt{K}, \overline{z}_K + \Phi^{-1}(1-\alpha/2) \hat{s}/\sqrt{K})$, where $\Phi(\cdot)$ is the cumulative distribution of a Normal(0,1), and $\alpha$ is a confidence level set by the user. Typically, $\alpha = 0.05$.
