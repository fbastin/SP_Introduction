...
%	\hspace{2cm}
%	\includegraphics[scale=0.07]{Fin-MLLogo.png}
}
\frame{\titlepage}
...
& \phantom{\text{s.t. }}x \in X \subset \calR^n.
\end{split}
\]
...
\[
\begin{split}
& \text{``}\min_{x \in X}\text{''} g_0(x,\bsxi) \\
...
\item
The farmer can sell the sugar beets at \$36/T for the first 6000 tons, and \$10/T thereafter, due to European quotas.
\end{itemize}
...
Production can however increase or decrease by 20\%, depending on the weather.
...
\hline
$s = 1$ & Production (T) & 510 & 288 & 6000 \\
& Sales (T) & 310 & 48 & 6000 \\
& Purchases (T) & - & - & - \\
\hline
$s = 2$ & Production (T) & 425 & 240 & 5000 \\
& Sales (T) & 225 & - & 5000 \\
& Purchases (T) & - & - & - \\
\hline
$s = 3$ & Production (T) & 340 & 192 & 4000 \\
& Sales (T) & 140 & - & 4000 \\
& Purchases (T) & - & 48 & - \\
\hline
\end{tabular}
...
\item
The difference with the optimal decision under uncertainty is called {\blue expected value of perfect information (EVPI)}: profit loss due to uncertainty (here, \$7016). 
\end{itemize}
...
\]
\item
$\mathcal{Q}(x)$ is the expected sale amount if the newsvendor buys $x$ newspapers:
\[
...
\item
Here $Q(x, \xi)$ is the amount of money obtained by the newsvendor if he buys $x$ newspapers and the demand is $\xi$.
\end{itemize}
...
Example: $c = 0.15$, $q = 0.25$, $r = 0.02$, $\bsxi \sim N(650, 80^2)$. Then
\[
x^* = N^{-1}_{(650,80^2)}(0.1/0.23).
\]
Since $\bsxi = 80 Z + 650$ where $Z \sim N(0,1)$ has distribution function $\Phi$, it is easy to show that
\[
...
{\textsl{Other interpretation}}, more intuitive: assume that the vendor has bought $t$ newspapers.
What is the expected marginal revenue if he buys an additional newspaper?
From an economic point of view, we would like this marginal revenue to be equal to 0.
