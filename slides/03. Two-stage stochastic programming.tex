...
\begin{frame}
	...
	\begin{align*}
	\min_{x,\ y(\bsxi)}\ & \EE_{\bsxi} [ c^T x + q^T y(\bsxi) ] \\
...
Therefore, a.s.
$$
Q(x,\xi(\omega)) \geq -(q^-)^Tz(\xi(\omega)),
$$
...
$$
\calQ(x) \geq \EE_{\bsxi}\left[-(q^-)^Tz(\bsxi)\right].
$$
\end{proof}
...
\begin{proof}
In order to show convexity in (a) and (c), it is sufficient to prove that $v(\xi, z) = \min_{y \in Y} \lbrace q(\xi)^Ty \ |\ Wy = z \rbrace$ is convex, which has already been done.
...
When the support of $\bsxi$ is finite this is immediate: $\mathcal{Q}$ is a non-negative weighted sum of the $Q(\cdot,\xi_s)$, each of which has just been shown to be convex and piecewise linear.
