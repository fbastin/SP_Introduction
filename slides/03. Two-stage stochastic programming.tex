...
\begin{frame}
	...
	\begin{align*}
	\min_{x,\, y}\ & x_1+x_2 + \sum_{s = 1}^{S} p_s \lambda (y_{1s} + y_{2s})\\
...
$$
P\left[ \lbrace \xi \ |\ Q(x,\xi) < \infty \rbrace \right] = 1 \Longrightarrow \mathcal{Q}(x) < \infty,
$$
and consequently
$$
K_2 = K_2^s.
...
Moreover,
\begin{align*}
v(\xi, z_{\alpha}) & = q(\xi)^Ty^*(z_{\alpha}) \leq
q(\xi)^T y_{\alpha} = q(\xi)^T(\alpha y_1^*+(1-\alpha)y_2^*) \\
...
Therefore {\red $Q(x, \xi)$ is convex} w.r.t. $x$, given $\xi$.
More generally
\begin{theo}
...
\]
Suppose for now that $\bsxi$ has a finite support, i.e. $\Xi = \lbrace \xi_1 ,
\xi_2,\ldots, \xi_S \rbrace$. Then
\[
...
Therefore $\mathcal{Q}(x)$ is not differentiable {\red everywhere}: it is
differentiable except at the finitely many kinks, where we will have to work
with subgradients.
