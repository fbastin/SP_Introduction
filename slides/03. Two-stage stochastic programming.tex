...
\begin{frame}
	...
	Consider the linear program (LP), parametrized by the random variable (r.v.) $\bsxi: \Omega \rightarrow \RR^2$:
	\begin{align*}
		\min_x\ & c^T x \\
		\mbox{s.t. } & Ax = b \\
		& T(\bsxi)x = h(\bsxi) \\
...
\begin{frame}
	...
	\begin{itemize}
		...
		\item
		But this approach is rarely appropriate!!! We usually have to decide on $x$ {\red before} we know the realizations of $\bsxi$!
		\item
		Usually, the ``wait-and-see'' approach is not appropriate to model the reality behavior: we have to decide on $x$ before we know the realizations from $\bsxi$.
		%		\item
...
\begin{frame}
	...
	A popular approach consists to look for reasonable values for $\bsxi_1$ and $\bsxi_2$.
	How?
...
\begin{frame}
	...
	More general, consider the following program:
	\begin{align*}
	\min\ & c^Tx+\EE_{\bsxi}[q^T(\bsxi)y(\bsxi)] \\
	\mbox{s.t. } & Ax = b, \\
...
\begin{frame}
	...
	\begin{align*}
	\min_{x,\ y(\bsxi)}\ & E_{\bsxi} [ c^T x + q^T y(\bsxi) ] \\
	...
	\omega_2,\ldots,\omega_S \rbrace \subseteq \RR^r$.
	\begin{align*}
...
\begin{frame}
	...
	\begin{itemize}
		...
		A Telecom company wantss to expand its network in order to meet an unknown (random) demand.
		\item
...
\[
\min c^Tx + E_{\bsxi}[q(\bsxi)^Ty(\bsxi)]
\]
...
y(\xi) & \in Y,
\end{align*}
where $\bsxi$ is a random vector defined on the random space 
...
\[
\mathcal{Q}(x) \overset{def}{=} \EE_{\bsxi}[Q(x,\bsxi)],
\]
...
$$
Note that $P\left[ \bsxi = 2^n \right] \in (0,1),\ n = 0,1,2,\ldots$, $\Xi = \lbrace 2^n,\ n \in \NN \rbrace$, and
$$
...
\begin{theo}
If $\bsxi$ has finite second order moments, then
$$
...
\]
\item
Clearly $K_2 = K_2^P$ if $\bsxi$ has a finite support.
...
By combining these two cases, we conclude that  $\mathcal{Q}$ is not finite.
\end{proof}
...
Any feasible solution satisfies $y^+(\xi(\omega))-y^-(\xi(\omega))=h(\xi(\omega))-T(\xi(\omega))x := z(\xi(\omega))$, with $y^+(\xi(\omega)) \geq 0$ and $y^-(\xi(\omega)) \geq 0$.
...
$$
so, a.s., $\exists$ a lower bound on $Q(x,\xi(\omega))$, and since the program is feasible, it admits an optimal solution.
...
%	\underline{Rappel}: pour un programme stochastique avec un recours%	fixe (pas le cas ici!) où $\bsxi$ a des moments seconds finis, $K_2 = K_2^P$.
...
More generally
\begin{theo}
If $A$ is a linear transformation from $\RR^n$ to $\RR^m$, and $f(x)$ is a convex function on $\RR^m$, the composite function $(fA)(x) \overset{def}{=} f(Ax)$ is a convex function on $\RR^n$.
\end{theo}
\end{frame}
...
\includegraphics[width=0.62\textwidth]{recourse_all.pdf}
...
As $\mathcal{Q}(x)$ is a finite weighted sum of piecewise linear functions when the support of $\bsxi$ is finite, we have the following result.

\begin{theo}
For a stochastic program with fixed recourses where $\bsxi$ has finite second-order moments,
\begin{enumerate}[(a)]
\item
...
But how to prove $(1/h) \int_{\Xi
  \backslash N_{\delta}} \rho_j (\hat{x}, \xi; h) dP \overset{h
  \rightarrow 0}{\rightarrow } 0$?

\mbox{}

If we stay in the linear framework with fixed recourse, and vectors $\bsxi$ with finite second-order moments, we have seen that for $\bsxi$ with finite support, $\mathcal{Q}(x)$ is piecewise linear.
...
\begin{theo}
For a stochastic program with fixed recourse where $\bsxi$ has finite second-order moments, if $\bsxi$ is continuous, $\mathcal{Q}(x)$ is differentiable over $K_2$.
\end{theo}
...
\[
\min_{x \in X} E_{\bsxi} [ f_0(x, \bsxi) ] = \min_{x \in X}
E_{\bsxi} [g_0(x,\bsxi) + Q(x, \bsxi)].
\]
