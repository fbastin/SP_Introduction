...
\begin{frame}
	...
	Uncertainty is represented by {\red random elements} on a probability space $(\Omega, \calF, P)$: a realization $\omega$ is drawn from the sample space $\Omega$, and an {\red event} is a member of the $\sigma$-algebra $\calF$.
	
	\mbox{}
	
	Throughout, {\red $\bsxi$} denotes a random vector, {\red $\xi$} one of its realizations, and {\red $\Xi$} its support --- the smallest closed set with $P[\bsxi \in \Xi] = 1$.
	
	\mbox{}
	
	{\footnotesize The {\red probability background notes} cover the whole setting used here, and in particular the four points this deck leans on without redefining them: the measurability that makes $\calQ(x)$ well defined, the closedness of the support, the meaning of ``finite second-order moments'', and the difference between ``almost surely'' and ``always''.}
...
\begin{frame}
	...
	\omega_2,\ldots,\omega_S \rbrace$, so that $\Xi = \bsxi(\Omega) \subseteq \RR^r$ --- the range
	coincides with the support here, every scenario having a positive probability, but not in general.
	\begin{align*}
...
Unless stated otherwise, $\bsxi$ {\red gathers the random entries} of $q$, $h$ and $T$, as in Birge and Louveaux.
This loses no generality, and the hypotheses made on $\bsxi$ below --- moments, absolute continuity --- are then hypotheses on the data themselves.
...
\begin{theo}
For a stochastic program with fixed recourse, where $\bsxi$ has finite second-order moments ($\EE[\|\bsxi\|^2] < \infty$; see the probability notes for why this is the hypothesis that recurs),
$$
...
\begin{frame}
\frametitle{$K_2$ is a polyhedron}
\small

\begin{theo}
For a stochastic program with fixed recourse where $\bsxi$ has a {\red finite support}, $K_2$ is a polyhedron.
\end{theo}

$\text{pos}W$ is a polyhedral cone, so it is the intersection of finitely many half-spaces through the origin: writing $\sigma^1, \ldots, \sigma^r$ for the generators of its {\red polar cone} (background notes),
\[
z \in \text{pos}W \iff (\sigma^k)^Tz \leq 0,\ k = 1, \ldots, r.
\]
With a fixed recourse, $x \in K_2(\xi) \iff h(\xi)-T(\xi)x \in \text{pos}W$, hence
\[
K_2(\xi) = \lbrace x \,|\, (\sigma^k)^T\left(h(\xi)-T(\xi)x\right) \leq 0,\ k = 1,\ldots,r \rbrace,
\]
a polyhedron. When $\Xi = \lbrace \xi_1,\ldots,\xi_S \rbrace$ is finite, $K_2 = \cap_{s=1}^S K_2(\xi_s)$ is a finite intersection of polyhedra, hence a polyhedron.

\mbox{}

{\footnotesize Those inequalities are exactly the {\red feasibility cuts} of the next chapter, which never enumerates the $\sigma^k$ but discovers, one at a time, the few that the current iterate violates. On an infinite support the intersection is no longer finite and $K_2$, while still closed and convex, need not be polyhedral.}
...
{\red This is where the finite second-order moments are used}: $\bsxi$ gathers the random entries of $q$, $h$ and $T$ (the convention stated with the fixed-recourse problem), and $z(\xi) = h(\xi)-T(\xi)x$ is linear in $(h, T)$, so $\EE[\|q(\bsxi)\|^2]$ and $\EE[\|z(\bsxi)\|^2]$ are finite, and Cauchy--Schwarz gives
$$
...
\begin{theo}
Assume that $W$, $T$ and $q$ are fixed, and that only the right-hand side is random, {\red affine} in $\xi$: $h(\xi) = h_0 + H\xi$ for a fixed vector $h_0$ and a fixed matrix $H$ (e.g.\ $h(\xi) = \xi$).
Then $\EV \leq \WS$.
\end{theo}
\begin{proof}
Set $\psi(\xi) \overset{\mathrm{def}}{=} \min_{x \in K_1 \cap K_2(\xi)} z(x,\xi)$.
Minimizing over $x$ and $y$ together, $\psi(\xi) = \varphi(h(\xi))$, where
\[
\varphi(r) \overset{\mathrm{def}}{=} \min_{x \geq 0,\, y \geq 0} \lbrace c^Tx + q^Ty \,|\, Ax = b,\ Tx + Wy = r \rbrace
\]
is the value function of a single linear program in its right-hand side, hence convex (LP notes): this is where $W$, $T$ and $q$ must be fixed. Then
\[
\EV = \varphi\big(h(\EE[\bsxi])\big) \ {\red =}\ \varphi\big(\EE[h(\bsxi)]\big) \leq \EE_{\bsxi}\big[\varphi(h(\bsxi))\big] = \WS.
\]
The {\red equality} is where affinity is used, $h(\EE[\bsxi]) = h_0 + H\,\EE[\bsxi] = \EE[h(\bsxi)]$, and the inequality is Jensen's (probability notes), for the random vector $h(\bsxi)$.
\end{proof}
\end{frame}

\begin{frame}
\frametitle{Neither assumption can be dropped}
\small

Take $c = 0$, $0 \leq x \leq 1$, $\bsxi = \pm 1$ with probability $1/2$ each, and a single second-stage constraint:
\[
Q(x,\xi) = \min_{y \geq 0} \lbrace y \,|\, y \geq h(\xi) - T(\xi)x \rbrace = \big(h(\xi) - T(\xi)x\big)^+.
\]
\begin{itemize}
\item
{\blue $h$ random, not affine}: $h(\xi) = 1 - \xi^2$, $T = 0$.
Then $\psi(\xi) = (1-\xi^2)^+$, so $\WS = 0 < 1 = \psi(0) = \EV$.
The red equality fails, $h(\EE[\bsxi]) = 1 \neq 0 = \EE[h(\bsxi)]$: the EV problem is built on the mean of $\bsxi$, not on the mean right-hand side.
\item
{\blue $T$ random}: $h = 1$, $T(\xi) = \xi$.
Then $\psi(1) = \min_x (1-x)^+ = 0$ and $\psi(-1) = \min_x (1+x)^+ = 1$, so $\WS = 1/2 < 1 = \psi(0) = \EV$.
Now $\xi$ multiplies $x$: $Q(x,\cdot)$ is still convex in $T$ for each fixed $x$, but the minimum over $x$ is not --- $\psi = 1$ on $[-1,0]$ and $1-\xi$ on $[0,1]$.
\end{itemize}
Collecting the results: $\WS \leq \RP \leq \EEV$ always holds, and when only $h(\bsxi)$ is random, affine in $\xi$,
\[
...
\begin{tikzpicture}[x=1cm,y=1cm,>=latex,font=\footnotesize]
  ...
  \node[below,font=\scriptsize,align=center,gray] at (1.8,-0.85)
     {guaranteed when only $h(\bsxi)$\\ is random, affine in $\xi$};
