...
\begin{frame}
	...
	and
	\begin{align*}
	Q(x, \xi) &= \min_y q_1y_1 + q_2y_2 \qquad (q_1 = q_2 = \lambda \text{ above}) \\
...
More generally
\begin{theo}
If $A$ is a linear transformation from $\RR^n$ to $\RR^m$, and $f(x)$ is a convex function on $\RR^m$, the composite function $(fA)(x) \overset{\mathrm{def}}{=} f(Ax)$ is a convex function on $\RR^n$.
\end{theo}
\end{frame}
