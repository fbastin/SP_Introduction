...
As $W$, $T$ and $q$ are fixed, $\psi(\xi)$ is the optimal value of a linear program whose only varying data is the right-hand side, so it is convex in $h(\xi)$ and hence in $\xi$ (LP notes). Jensen's inequality (probability notes) then gives
\[
