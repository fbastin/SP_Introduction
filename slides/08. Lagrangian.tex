...
\begin{corollary}
If the recourse feasible set is bounded for any $x \in \RR^{n_1}$, $\mathcal{Q}$ is a lower semi-continuous function.
\end{corollary}
...
Under the assumption of uniqueness of dual solution, we can show that this algorithm always produces an {\blue ascent direction} in $\theta$.
\item
...
$\mathcal{N}$ designs the closed linear subspace of nonanticipative processes.
...
\EE\left[\sum_{t = 0}^T f_{t+1} (\bsxi, x^t(\xi), x_{t+1}(\bsxi) \right] \\
...
\begin{align*}
\theta(\pi) = \inf_{x \in \mathcal{X}} z = & 
\EE\left[\sum_{t = 0}^T f_{t+1} (\xi, x^t(\xi), x_{t+1}(\xi) \right] \\
...
\end{align*}
$\rho$ if the augmented Lagrangian penalty parameter, and $\pi_s$ is the vector of dual variables associated to the nonanticipativity constraints of the scenario  $s$.
...
\begin{itemize}
\item
Step 2 simply consists of taking the expected value of $x^{\nu+1}$
as $\hat{x}^{\nu+1}$.
\item
...
\begin{description}
	\item[Step 0.]
	Set $\hat{x}^{(s) ,0} = \left(\hat{x}_1^{(s),0}, \dots, \hat{x}_T^{(s),0}\right)$ and $k = 0$. Choose $\pi^{(s),0} = \bf{0}$, $\rho^0 > 0$.
	\item[Step 1.]
	...
	\begin{aligned}
	\min_{x^{(s)}} \ &  f\left( x^{(s)}, \xi^{(s)} \right) + \sum_{t = 1}^T \left( {\pi_t^{(s)}}' \biggl(x_t^{(s)} - \hat{x}_t^{(s),k}\right) \\
...
The progressive hedging algorithm converges to an optimal solution $x^*, \pi^*$.
\end{theorem}
