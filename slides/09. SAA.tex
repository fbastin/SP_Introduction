...
If $\bxi$ cdf is continuous or discrete with a large number of possible realizations, $g(z)$ is usually very difficult to evaluate.

\mbox{}

We have to use some approximations such as the Monte Carlo (MC) methods: the original problem is replaced by successive approximations obtained by drawing $\xi_1,\ldots{}, \xi_n$:
...
This framework includes the two-stage programs that we have analyzed, as
\begin{align*}
\min_x\ & c^Tx + \EE[\cQ(x)] \\
...
$(\xi_1,\ldots,\xi_n)$ can be seen as the finite truncation of an infinite sequence $\overline{\xi} := \lbrace \xi_k \rbrace_{k =   1}^{\infty}$.
...
{\blue Under which conditions is such a limit point a first-order critical point for the true problem?}
...
\]
later called the {\it sampling process}, where the random vectors $\bxi_k$, $k = 1,\ldots, \infty$, are assumed to be independent and identically distributed (i.i.d.).
...
From the i.i.d. property and the Kolmogorov consistency theorem (see for instance Parthasarathy, {\sl Probability Measures on Metric   Spaces}, Academic Press, 1967, Chapter~V, Theorem 5.1), we can build the probability space of infinite dimension
\[
...
In other words, the marginal measures defined on $\prod_{i = 1}^j (\Xi, \mathcal{F})$, with finite $j$, $j \geq 1$, correspond to the product measures $\prod_{i = 1}^j P$, as expected.
...
\noindent
\textbf{A.2}
The family $G(z, \xi)$, $z \in S$, is dominated by a $P$-integrable function $K(\xi)$, i.e. $\EE_P[K]$ is finite and $| G(z, \xi) | \leq K(\bxi)$ for all $z \in S$ and $P$-almost each $\xi$.

\end{frame}

\begin{frame}
\frametitle{Do we need {\bf A.2}?}

{\bf A.2} is not implied by {\bf A.1}.

\mbox{}

Consider $\bxi$ s.t.
$$
P[\xi = n] = \frac{C}{n^5},\ n = 1, 2, 3,\ldots,
$$
where
$$
C = \frac{1}{\sum_{n = 1}^{\infty} n^{-5}} = \frac{1}{\zeta(5)} \approx 0.9644
$$
We have
$$
\EE[\xi^2] = C \sum_{n = 1}^{\infty} n^2\frac{1}{n^5} = C \sum_{n = 1}^{\infty}\frac{1}{n^3} = \frac{\zeta(3)}{\zeta(5)} < \infty
$$

\end{frame}

\begin{frame}
\frametitle{{\bf A.1} but not {\bf A.2}}
...
Then
$$
G(z,n) = n^4 z \in C^1 \text{ on } S.
$$
So {\bf A.1} holds. Moreover
$$
\sup_{z \in S} | G(z,\bxi) | = \sup_{z \in S} \bxi^4 z = \bxi^4.
$$
Hence any candidate $K(\bxi)$ must satisfy $K(\bxi) \geq \bxi^4$. Thus
$$
\EE [K(\bxi)] \geq \EE \left[ \bxi^4 \right] = C \sum_{n = 1}^{\infty} n^4\frac{1}{n^5} = C \sum_{n = 1}^{\infty}\frac{1}{n} = +\infty.
$$
...
This last property and \textbf{A.2}, are typical assumptions in stochastic programming.
\mbox{}
...
\begin{footnotesize}
The law of large numbers, its weak and strong forms, and the modes of convergence they use are recalled in the {\red probability background notes}. The ULLN strengthens it from pointwise to uniform convergence over $S$, which is what optimization needs.
\end{footnotesize}

\mbox{}

Proof: see Theorem 7.48 in \href{https://bpb-us-e1.wpmucdn.com/sites.gatech.edu/dist/4/1470/files/2021/03/SPbook.pdf}{Lecture notes on stochastic programming}.
...
%\] and
\item
$\hat{\phi}_n(z)$ converges to $\phi(z)$ almost surely uniformly on $\mathcal{S}$ as $n \rightarrow \infty$.
\end{enumerate}
...
A subset $C$ of a vector space $V$ is a (linear) {\red cone}
iff $\alpha x$ belongs to $C$ for any $x \in C$ and any strictly positive scalar $\alpha$;
allowing $\alpha = 0$ adds the apex, $0 \in C$.
...
{\footnotesize Not to be confused with a {\red pointed} cone, one containing no line, $C \cap -C = \lbrace 0 \rbrace$ --- the property giving a polyhedron its extreme points, and the sense the background notes use.}
...
The {\red normal cone $N_{\mathcal{A}}(x)$} is its {\red polar cone} (background notes), i.e.
\[
N_{\mathcal{A}}(x) = \lbrace v\ |\ v^Tw \leq 0,\ \forall w \in T_{\mathcal{A}}(x) \rbrace.
\]

{\footnotesize The definition above is that of the {\red Clarke} tangent cone; the Bouligand one requires only that {\it some} such sequences exist. The two coincide when $\mathcal{A}$ is convex, which is the only case used below.}
...
\begin{itemize}
	\item 
Thus, when $S$ is convex, we can rewrite the first-order criticality conditions at $z^*$ as 
$$
...
Consider %the choice
$\Gamma(\cdot) = \mathcal{N}_S(\cdot)$; $\phi(z^*)$ belongs to $\Gamma(z^*)$ iff
\[
\langle \phi(z^*), u-z^*\rangle \leq 0,\ \forall u \in S.
\]
\item
We refer to such variational inequalities as \textcolor{red}{stochastic variational inequalities}.
\end{itemize}
...
\item
Assumption (d) follows from the ULLN, while \textbf{A.1} and the compactness of $S$ ensure assumption (c) by setting $\mathcal{S} = S$. 
\end{itemize}

The previous theorem therefore ensures the first-order criticality at the limit as $n\rightarrow\infty$, almost surely.
...
%accumulation points.
The compactness of $V$ implies that for each $\overline{\xi}$, %the set
...
\begin{itemize}
	\item 
If $\exists M > 0$ s.t. $\| \lambda^*_N (\overline{\xi}) \|$ for all $N$, 
$\lbrace \lambda^*_{\ell} (\overline{\xi}) \rbrace_{\ell = 1}^{\infty}$ admits a limit point.
\item 
This also allows the use of stochastic variational inequalities (Shapiro, 2003).
\end{itemize}

\end{frame}

\begin{frame}
\frametitle{Stochastic variational inequalities for KKT}

Let $u := (z,\lambda) \in \rit^{m+M}$ and $\mathcal{K} := \rit^m
...
\begin{align*}
	...
	\bepsilon_n)\right).
\end{align*}
The generalized equation $\phi(u) \in \mathcal{N}_{\mathcal{K}}(u)$ then represents the KKT optimality conditions for the original optimization problem, $S^* \subseteq \overset{o}{V}$, and the theorem on generalized equations then implies first-order criticality almost surely, with $\Gamma(u) := \mathcal{N}_{\mathcal{K}}(u)$.

Assumptions $(a)$ and $(d)$ of the theorem on generalized equations are sufficient as $\bepsilon \rightarrow 0$ a.s., and assumption $(c)$ holds as feasible sets for original and approximate problems are non-empty.
...
In other words,
\[
\exists\, \delta_n(\overline{\xi}) \text{ s.t. } \forall z \in
...
%In order to show that $z^* \in \mathcal{Z}\left(\lbrace z^*_n(\overline{\xi})\rbrace\right)$ is a local minimizer of $g(\cdot)$ over $S$,  we have to show that for some subsequence
{\blue Idea:} given $\lbrace z^*_{\ell} (\overline{\xi}) \rbrace_{\ell = 1}^{\infty} \subseteq \lbrace z^*_{n} (\overline{\xi}) \rbrace_{n = 1}^{\infty}$ s.t. $\lbrace z^*_{\ell} \rightarrow z^*$, the neighborhood where $z_{\ell}^*(\overline{\xi})$ is a local minimizer do not reduce to a singleton as $\ell \rightarrow \infty$.
...
\begin{proof}
See Bastin, Cirillo, and Toint (2006), {\sl Convergence theory for
	nonconvex stochastic programming with an application to mixed logit},
Mathematical Programming, 108(2--3):207--234.
\end{proof}
...
Unlike classical sensitivity analysis, we focus here on conditions under which a limit point of a sequence of approximate solutions is a solution of the true problem.
...
Under some conditions, if the subsequence $\lbrace z_{\ell}^* \rbrace_{\ell =  1}^{\infty} \rightarrow z^*$, then %the sequence of associated Lagrange multipliers vectors
$\lbrace \lambda_{\ell}^* \rbrace %$ also converges to some Lagrange multipliers vector $
\rightarrow \lambda^*$, associated to $z^*$. % for the true problem.
...
\begin{defi}
Consider $z^*$ and the active set $\mathcal{A}(z^*)$:
...
\]
The LICQ holds at $z^*$ if the set of active constraints gradients $\lbrace \nabla c_j(z^*),\ j \in \mathcal{A}(z^*) \rbrace$ is linearly independent.
\end{defi}
...
\begin{defi}
Given $z^*$ and a vector $\lambda^*$ satisfying the KKT conditions, the strict complementarity condition holds if %exactly one of $[\lambda]_j^*$ and $c_j(z^*)$ is null for every index $j=1,\ldots,k$, i.e. we have that 
$[\lambda]_j^* > 0$ for each $j \in \lbrace 1,\ldots,k \rbrace \cap \mathcal{A}(z^*)$.
\end{defi}

\mbox{}

Consider again a particular sampling process $\overline{\xi}$ in $(\Xi_{\Pi}, \mathcal{F}_{\Pi}, P_{\Pi})$, such that $\epsilon_n (z, \overline{\xi} ) \rightarrow 0$ as $n \rightarrow \infty$. 
...
Assume that, for almost every sampling process $\overline{\xi}$ in $(\Xi_{\Pi}, \mathcal{F}_{\Pi}, P_{\Pi})$, there exists some $z^* \in \mathcal{Z} \left(\lbrace
  z^*_n(\overline{\xi})\rbrace\right) \cap \overset{o}{V}$ associated to a unique vector of Lagrange multipliers $\lambda^*$ and some subsequence $\lbrace z_{\ell}^* \rbrace_{\ell = 1}^{\infty} \subseteq \lbrace  z_n^* \rbrace_{n = 1}^{\infty}$, such that $z^*_{\ell}(\overline{\xi}) \rightarrow z^*$, and
\begin{enumerate}[(a)]
\item
...
\|w\|=1.
\end{multline*}
This is equivalent to saying that $z^*$ is an isolated minimizer for the original problem.
\end{theo}
...
Remark that the LICQ and the strict complementarity condition imply that the minimizer is isolated while sufficient second-order conditions are usually used to characterize strict local minimizers.
...
\begin{frame}
\frametitle{Second-order convergence: corollary}

\begin{coro}
Assume that, for almost every sampling process $\overline{\xi}$ in $(\Xi_{\Pi}, \mathcal{F}_{\Pi}, P_{\Pi})$, there exist some $z^* \in \mathcal{Z} \left(\lbrace z^*_n(\overline{\xi})\rbrace\right) \cap \overset{o}{V}$ associated to a unique Lagrange multipliers vector $\lambda^*$ and some subsequence $\lbrace z_{\ell}^* \rbrace_{\ell = 1}^{\infty} \subseteq \lbrace  z_n^* \rbrace_{n = 1}^{\infty}$, such that $z^*_{\ell}(\overline{\xi}) \rightarrow z^*$, that assumptions (a)--(d) of the second-order convergence theorem hold and that the strict complementarity condition holds at $( z^*, \lambda^* )$ for the original program.

\mbox{}

Assume moreover that the Jacobian of the equality relations of the KKT conditions is non-singular at $( z^*, \lambda^* )$.
Then there exists almost surely some $z^*$ in $\mathcal{Z} \left( \lbrace z_N^*(\overline{\bxi}) \rbrace \right) \cap \overset{o}{V}$, associated to some vector $\lambda^*$, that satisfies the sufficient second-order conditions of the original program.
\end{coro}
...
%The inverse implication of the second-order convergence theorem can be obtained from classical results in perturbations analysis (A.~V. Fiacco, {\sl Introduction to Sensitivity and  Stability Analysis in Nonlinear Programming}, Academic Press, New York, USA (1983), Theorem~3.2.2).
%More developments if the context of stochastic programming can be found in  Rubinstein and Shapiro (2003) and A.~Shapiro, {\sl Probabilistic constrained optimization: Methodology and applications}, in S.~Uryasev, editor, {\sl Statistical inference of stochastic optimization problems}, pages 282--304. Kluwer Academic Publishers, 2000.
...
In other words, for $N$ finite, the optimal value estimator is biased, but asymptotically unbiased.
...
\begin{center}
\includegraphics[width=0.68\textwidth]{instance_landing.pdf}
\end{center}
\begin{small}
...
\begin{frame}
	...
	\begin{quote}
		B.K. Pagnoncelli, S. Ahmed, and A. Shapiro (2009), ``Sample Average Approximation Method for Chance
		Constrained Programming: Theory and Applications'', Journal of Optimization Theory and Applications 142 (2), pp. 399--416.
	\end{quote}
...
\begin{frame}
	...
	The next slides are due to Thuy Anh Ta.
