...
\begin{itemize}
	\item 
	\textbf{A.3} implies that $\EE_P[g(z)]$ is continuously ddifferentiable on $S$, and
...
\begin{defi}
Given $z^*$ and a vector $\lambda^*$ satisfying the KKT conditions, the strict complementarity condition holdss if %exactly one of $[\lambda]_j^*$ and $c_j(z^*)$ is null for every index $j=1,\ldots,k$, i.e. we have that 
...
Assume that the strict complementarity condition holdss at $z^*$ for the approximate problem.
...
z^* )$, so the strict complementarity condition holdss at $\left(
