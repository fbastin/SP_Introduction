...
The recent years have seen a huge success of machine learning, and a renewed interest in the stochastic gradient algorithm, and the development of numerous variants.
...
It extends easily to unconstrained optimization, the first-order necessary condition for
$$
\min_{x \in \RR^d} \ f(x)
\qquad \text{being} \qquad
\nabla_x f(x) = 0 .
$$
...
We can also restrict the feasible domain to $\Theta \subseteq \RR^d$:
$$
...
\nabla_x \EE[Y(x,\xi)] = 0
$$
We assume here that we can exchange the expectation and differentiation operators, i.e.
$$
\nabla_x \EE[Y(x,\xi)] = \EE[\nabla_x Y(x,\xi)]
$$
...
Also known as the \textcolor{red}{stochastic gradient descent (SGD) method}.
...
Therefore $X_n = o_p(1) \Rightarrow X_n = O_p(1)$; the reverse does not hold.
...
Suppose the number of iterations completed in $c$ units of computer time, $n(c)$, grows roughly linearly in $c$ --- as it would if sample gradients were computed in constant time.
...
of $A(\cdot)$ on the initial iterate $x$.
...
\begin{enumerate}
\item
$\EE[ Y^2 ( x , \xi)] < \infty$ for all $x \in \Theta$.
\item
...
While we assume the draws $\xi_i$, $i = 1,2,\ldots$ are i.i.d., it is possible to extend the results to the situation where $\lbrace \xi_i, i = 1,2,\ldots \rbrace$ form an adapted stochastic process, where each $\xi_i$ can depend on the previous ones.
...
\begin{frame}
	...
	This implies
    $$
		\EE_{\xi_k} [f(x_{k+1}) - f(x_k)] \leq \EE_{\xi_k} \left[ - \alpha_k \nabla f(x_k)^T g(x_k, \xi_k) + \alpha_k^2 \frac{L}{2} \| g(x_k, \xi_k) \|^2_2 \right]
	$$
	or
	$$
		\EE_{\xi_k} [f(x_{k+1})] - f(x_k) \leq - \alpha_k \nabla f(x_k)^T \EE_{\xi_k} [ g(x_k, \xi_k) ] + \alpha_k^2 \frac{L}{2} \EE_{\xi_k} \left[ \| g(x_k, \xi_k) \|^2_2 \right].	
	$$
	
\end{frame}

\begin{frame}
\frametitle{Assumptions: first and second moment limits}

The SG method applied to $f(\cdot)$ satisfies
\begin{enumerate}[a)]
\item
There exists an open set $\calA$ such that
$$
f(x) \geq f_{lb}(x),\ \forall\, x \in \calA,
$$
and
\begin{equation}
x_k \in \calA,\ \forall k \in \NN
\tag{A.5}
\label{ass:open_set}
\end{equation}
\item
...
&= \EE_{\xi_k} \left[ \| g(x_k, \xi_k) \|^2_2 \right] - \left\| \EE_{\xi_k} [ g(x_k, \xi_k) ] \right\|^2_2 \\
&\leq M + M_V \| \nabla f(x_k) \|_2^2.
\end{aligned}
\tag{A.7}
\label{ass:varg}
...
\begin{itemize}
\item
\ref{ass:sufficient_descent} expresses that in expectation, $g(x_k, \xi_k)$ is a sufficient descent direction.
\begin{itemize}
	\item True if $\EE_{\xi_k} [g(x_k, \xi_k)] = H_k \nabla f(x_k)$ with $H_k$ positive definite and bounded spectrum.
	\item Particular case: $H_k = I$. Then \ref{ass:sufficient_descent} holds with $\mu = \mu_G = 1$. 
\end{itemize}
\item 
\ref{ass:varg} is a direct generalization of \ref{ass:var}.
\item
From \ref{ass:sufficient_descent} and \ref{ass:varg},
\begin{align*}
\EE_{\xi_k} \left[ \| g(x_k, \xi_k) \|^2_2 \right]
& \leq \left\| \EE_{\xi_k} [ g(x_k, \xi_k) ] \right\|^2_2 + M + M_V \| \nabla f(x_k) \|_2^2 \\
...
\begin{theorem}[Moments]
	Under Assumptions~\ref{ass:Lipschitz} and \ref{ass:open_set}--\ref{ass:varg}, $\forall k \in \NN$,
	\begin{align*}
		\EE_{\xi_k}[f(x_{k+1})] - f(x_k)
		& \leq - \mu \alpha_k \| \nabla f(x_k) \|^2
		+ \frac{1}{2} \alpha_k^2 L \EE_{\xi_k} \left[ \| g(x_k, \xi_k )\|_2^2 \right] \\
...
$$
	\EE_{\xi_k} [f(x_{k+1})] - f(x_k) \leq - \alpha_k \nabla f(x_k)^T \EE_{\xi_k} [ g(x_k, \xi_k) ] + \alpha_k^2 \frac{L}{2} \EE_{\xi_k} \left[ \| g(x_k, \xi_k) \|^2_2 \right].	
$$
From \ref{ass:sufficient_descent}, this leads to
$$
\EE_{\xi_k} [f(x_{k+1})] - f(x_k) \leq - \alpha_k \mu \| \nabla f(x_k) \|_2^2 + \alpha_k^2 \frac{L}{2} \EE_{\xi_k} \left[ \| g(x_k, \xi_k) \|^2_2 \right],
$$
...
\end{frame}
	...
	\begin{frame}{Hierarchy of Convexity}
	\begin{center}
		\begin{tikzpicture}[scale=1.2]
			% Boxes
			\node[draw, rectangle, minimum width=4cm, minimum height=1cm, fill=blue!10] (strong) at (0,0) {Strongly Convex};
			\node[draw, rectangle, minimum width=5cm, minimum height=1cm, fill=blue!20] (strict) at (0,1.5) {Strictly Convex};
			\node[draw, rectangle, minimum width=6cm, minimum height=1cm, fill=blue!30] (convex) at (0,3) {Convex};
			
			% Arrows
			\draw[->, thick] (strong) -- (strict);
			\draw[->, thick] (strict) -- (convex);
			
			% Labels
%			\node[right] at (3.5,0.75) {$\subset$};
%			\node[right] at (3.5,2.25) {$\subset$};
		\end{tikzpicture}
	\end{center}
	
	\vspace{0.5cm}
	\textbf{Implication chain:}
	\[
...
\begin{columns}
	...
	\begin{itemize}
		\item[$\checkmark$] Convex
		\item[$\times$] Strictly convex
		\item[$\times$] Strongly convex
	\end{itemize}
	
	\vspace{0.3cm}
	Linear functions satisfy convexity with equality, but not the strict inequalities.
\end{columns}
\end{frame}
...
\begin{columns}
	...
	\begin{itemize}
		\item $\checkmark$ Convex
		\item $\checkmark$ Strictly convex
		\item $\checkmark$ Strongly convex ($m=2$)
	\end{itemize}
	
	\vspace{0.3cm}
	The second derivative $f''(x) = 2 > 0$ ensures strong convexity.
\end{columns}
\end{frame}

\begin{frame}{Example 3: Quartic Function}
\begin{columns}
	\column{0.5\textwidth}
	\begin{center}
		\begin{tikzpicture}[scale=0.8]
	...
	\vspace{0.3cm}
	$f''(0) = 0$ means no uniform lower bound on curvature. Strictly convex but not strongly convex.
\end{columns}
\end{frame}

\begin{frame}{Example 4: Exponential Function}
\begin{columns}
	\column{0.5\textwidth}
	\begin{center}
		\begin{tikzpicture}[scale=0.8]
	...
	\begin{itemize}
		\item $\checkmark$ Convex
		\item $\checkmark$ Strictly convex
		\item $\times$ Strongly convex (globally)
	\end{itemize}
	
	\vspace{0.3cm}
	Strictly convex since $f''(x) = e^x > 0$, but $f''(x) \to 0$ as $x \to -\infty$.
\end{columns}
\end{frame}

\begin{frame}{Example 5: Piecewise Linear Function}
\begin{columns}
	\column{0.5\textwidth}
	\begin{center}
		\begin{tikzpicture}[scale=0.8]
	...
	\vspace{0.3cm}
	Convex but has linear segments where strict inequality fails.
\end{columns}
\end{frame}
...
\begin{itemize}
	...
	\item But a strong local minimizer does not imply local strong convexity. Example: $f(x) = x^4$.
\end{itemize}
...
\begin{itemize}
\item \( \EE_k \): expectation w.r.t. \( \bsxi_k \).
\item \( \EE \): full expectation w.r.t. all \( \bsxi_1,\ldots,\bsxi_k \):
$$
\EE [ x_{k+1} ] =
\EE_{\bsxi_1}[\EE_{\bsxi_2}[\ldots\EE_{\bsxi_k}[x_{k+1}]]]
$$
\end{itemize}
	
\end{frame}

\begin{frame}{SG with fixed stepsize}

\begin{theorem}
Under Assumptions~\ref{ass:Lipschitz}, \ref{ass:open_set}--\ref{ass:varg}, and \ref{ass:strong_convexity}, if we apply the SGD with a \textcolor{red}{fixed stepsize} $\alpha_k = \overline{\alpha}$ satisfying
\begin{equation}
0 < \overline{\alpha} \leq \frac{\mu}{LM_G},
\label{eq:fixed_stepsize}
\end{equation}
...
& \overset{k \rightarrow \infty}{\longrightarrow} \frac{\overline{\alpha}LM}{2c\mu}.
\end{align*}
\end{theorem}

\begin{itemize}
	\item Converges only to a neighborhood of optimum.
\end{itemize}
...
The moments theorem, with $\alpha_k = \overline{\alpha}$ and \eqref{eq:fixed_stepsize}, gives
\begin{align*}
\EE_{\xi_k}[f(x_{k+1})] - f(x_k)
	& \leq - \overline{\alpha}\left( \mu -\frac{1}{2}\overline{\alpha} LM_G \right) \| \nabla f(x_k) \|_2^2
	+ \frac{1}{2} \overline{\alpha}^2 LM \\
	& \leq -\frac{1}{2} \overline{\alpha} \mu \| \nabla f(x_k) \|_2^2
	+ \frac{1}{2} \overline{\alpha}^2 LM.
\end{align*}
From strong convexity, this implies
$$
\EE_{\xi_k}[f(x_{k+1})] - f(x_k) \leq
- \overline{\alpha} c \mu \left( f(x_k) - f^* \right) + \frac{1}{2} \overline{\alpha}^2 LM,
$$
or equivalently
$$
\EE_{\xi_k}[f(x_{k+1})] - f^* \leq
f(x_k) - f^* - \overline{\alpha} c \mu \left( f(x_k) - f^* \right) + \frac{1}{2} \overline{\alpha}^2 LM.
$$
...
Thus,
$$
\EE \left[ \EE_{\xi_k}[f(x_{k+1})] - f^* \right] \leq
(1 - \overline{\alpha} c \mu) \EE \left[ f(x_k) - f^* \right] + \frac{1}{2} \overline{\alpha}^2 LM,
$$
or
\begin{align*}
\EE \left[ f(x_{k+1}) - f^* \right] - \frac{\overline{\alpha} LM}{ 2c \mu}
& \leq
(1 - \overline{\alpha} c \mu) \left( \EE \left[ f(x_k) - f^* \right] - \frac{\overline{\alpha} LM}{ 2c \mu} \right) \\
& \leq
(1 - \overline{\alpha} c \mu)^{k-1} \left( \EE \left[ f(x_1) - f^* \right] - \frac{\overline{\alpha} LM}{ 2c \mu} \right) \\
& \overset{k \rightarrow \infty}{\longrightarrow} \frac{\overline{\alpha}LM}{2c\mu},
\end{align*}
as
$$
0 < \overline{\alpha} c \mu \leq \frac{c\mu^2}{LM_G}
\leq \frac{c\mu^2}{L\mu^2} \leq 1.
$$
...
\begin{itemize}
	\item If noise \(M=0\), recovers linear convergence.
	\item If \(M>0\), noise limits accuracy.
\end{itemize}
	
\end{frame}

%\begin{frame}{SG with diminishing stepsizes}
%	\begin{itemize}
%		\item Halving \(\alpha\) eventually halves \(F(w_k)-F^\star\).
%		\item Overall rate: \( O(1/k) \).
%	\end{itemize}
%\end{frame}

%\begin{frame}{Minibatching}
%	Using minibatches with stepsize \( \alpha_k \):
	
%	\begin{itemize}
%		\item Variance decreases as \(1/b\) (batch size).
%		\item Implementation advantages on GPUs.
%	\end{itemize}
%\end{frame}
