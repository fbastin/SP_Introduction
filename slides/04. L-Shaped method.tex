...
\[
K_2 = K_2^s = \lbrace x \mid \mathcal{Q}(x) < \infty \rbrace.
\]
\end{itemize}
...
\begin{itemize}
\item
$y_s \overset{\mathrm{def}}{=} y(\omega_s)$ is the recourse action to take if scenario $s$ occurs.
\item
...
$n + pS$ variables;
\item
$m_1 + m_2 S$ constraints.
\end{itemize}
\item
The constraints have a very specific structure (staircase).
\item
...
\[
Q(\hat{x}, \xi_s) =
\max_{\pi} \lbrace \pi^T(h(\xi_s)-T(\xi_s)\hat{x}) \mid \pi^TW \leq q(\xi_s)^T \rbrace.
\]
Therefore, $\exists\, \hat{\pi}$ s.t.
$Q(\hat{x}, \xi_s) = \hat{\pi}^Th(\xi_s)-\hat{\pi}^TT(\xi_s)\hat{x}$.
For any $x \in K_2(\xi_s)$, by weak duality,
\[
Q(x, \xi_s) \geq \hat{\pi}^Th(\xi_s)-\hat{\pi}^TT(\xi_s) x,
