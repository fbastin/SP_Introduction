...
\[
W^Tu \geq 0 \mbox{ and } (h - T\hat{x})^T u < 0.
\]
or, by setting $\sigma = -u$,
\[
W^T\sigma \leq 0 \mbox{ and } (h - T\hat{x})^T \sigma > 0.
\]
...
For $S$, we can proceed as before, by replacing $\mathcal{Q}(x)$ with $\theta$: single-cut.
...
\Rightarrow \exists\, \sigma \in \RR^m \mbox{ s.t. } W^T\sigma \leq 0
\mbox{ and } (h-T\hat{x})^T\sigma > 0.
\end{multline*}
...
\begin{frame}
	...
	& y \geq 0,\ v \geq 0.
	\end{align*}
	The optimal value is $w^* = 11.2$, associated to $v^+_3 = 4.8$ and $v^+_5 = 6.4$, and the dual vector (obtained with glpk)
	\[
...
\begin{frame}
	...
	We can however show that the optimality cuts are supporting hyperplanes of $\mathcal{Q}(x)$, by means of subgradients, and the feasibility cuts adequately define the feasible points in $K_2$.
...
\begin{frame}
	...
	\[
	Q(\hat{x}, \xi_s) =
	\max_{\pi} \left\lbrace \pi^T(h(\xi_s)-T(\xi_s)\hat{x}) \,|\, \pi^TW \leq q(\xi_s)^T \right\rbrace.
	\]
	...
	Similarly, by taking the expectation, $- \sum_s p_s T(\xi_s)^T\hat{\pi}_s \in \partial \mathcal{Q}(\hat{x})$.
...
\begin{itemize}
	...
	\begin{itemize}
		\item 
	Multi-cut: more detailed information, so in general, we can expect less major iterations, but as more cuts are added, the first-stage program is more expensive to solve.
	\item
...
\begin{align*}
\mathcal{B}_0 &= \lbrace \RR^n_+ \times \lbrace \theta_1,
\theta_2,\ldots, \theta_{|S|}\rbrace \,|\, Ax = b \rbrace, \\
\mathcal{B}_1 &= \lbrace \RR^n_+ \times \lbrace \theta_1,
...
\[
\min c^Tx + \sum_{k = 1}^C \theta_k \mbox{ s.t. } (x, \theta_1,
...
\[
\min_x c^Tx + \sum_{j \in C} \theta_j + \frac{1}{2\rho} \| x-x_{\nu} \|^2.
\]
\begin{itemize}
\item
...
At iteration $k$, we require $\| x-x_k \|_k \leq \Delta_k$, where $\| \cdot \|_k$ denotes a standard norm (typically 2 or $\infty$).
...
\begin{itemize}
	...
	\item Constrain the candidate iterate to stay in a trust region $\calB_{\nu}$ centered at the current solution $x_{\nu}$:
$$
\cB_{\nu} = \{ x \,|\, \| x-x_{\nu} \|_{\nu} \leq \Delta_{\nu} \},
$$
...
Enforce $\Delta_{{\nu}+1} < \Delta_{\nu}$ if $\Delta_{\nu}$ has been kept the same for several consecutive unsuccessful iterations or if $\rho \Delta_{\nu}$ it too large.
\item
