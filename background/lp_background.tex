...
\begin{frame}
	...
	\begin{proof}
		...
		\]
		the first inequality because $\lambda \geq 0$ and $Ax \geq b$ (an equality in the asymmetric form, where $Ax = b$), the second because $x \geq 0$ and $A^T\lambda \leq c$.
	\end{proof}
...
\begin{tikzpicture}[x=1.25cm,y=1cm,>=latex,font=\footnotesize]
  ...
  \node[blue,above,font=\scriptsize] at (2.6,0.55) {$\lambda^Tb$, $\lambda$ dual feasible};
  ...
  \node[below,font=\scriptsize,gray] at (4.3,-0.45) {duality gap $c^Tx - \lambda^Tb \geq 0$ (weak duality)};
  \draw[dashed,gray] (4.3,0) -- (4.3,1.55);
  \node[above,font=\scriptsize,align=center] at (4.3,1.5) {$c^Tx^* = \lambda^{*T}b$\\{\scriptsize (strong duality)}};
\end{tikzpicture}
\end{center}
