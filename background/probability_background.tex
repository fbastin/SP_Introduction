...
\begin{frame}
\frametitle{The support}

The {\red support} $\Xi \subseteq \RR^r$ of $\bsxi$ is the {\red smallest closed set} carrying all the probability, $P[\bsxi \in \Xi] = 1$. Equivalently, and more usable in practice,
\[
\Xi = \lbrace x \in \RR^r \,|\, P[\bsxi \in V] > 0 \ \text{ for every open } V \ni x \rbrace :
\]
a point belongs to the support when no neighbourhood of it can be discarded.

\mbox{}

That a {\it smallest} such set exists is what the second form guarantees: the complement of $\Xi$ is a union of open sets of probability zero, and in $\RR^r$ countably many of them already cover it, so the complement itself has probability zero.

\mbox{}

{\footnotesize The {\red closedness} is not a detail either. It is exactly what makes the argument of deck~03 work: there, the set of $\xi$ favourable to a given $x$ is closed and has probability one, and must therefore contain the whole support --- which is how $K_2$ and $K_2^P$ come to coincide under a fixed recourse. Note also that $\Xi$ need not be connected, nor equal to the range $\bsxi(\Omega)$: a value taken with probability zero is not in the support.}
...
\begin{frame}
\frametitle{The distribution function}
\small

For a random variable, $F_{\bsxi}$ takes values in $[0,1]$, is non-decreasing, and tends to $1$ at $+\infty$ and to $0$ at $-\infty$.
It is moreover {\red right-continuous}, with left limits:
\[
\lim_{y \downarrow x} F_{\bsxi}(y) = P[\bsxi \leq x] = F_{\bsxi}(x),
\qquad
F_{\bsxi}(x^-) \overset{\mathrm{def}}{=} \lim_{y \uparrow x} F_{\bsxi}(y) = P[\bsxi < x],
\]
since the events $\lbrace \bsxi \leq y \rbrace$ decrease to $\lbrace \bsxi \leq x \rbrace$ in the first case, and increase to $\lbrace \bsxi < x \rbrace$ in the second.
The {\red jump} of $F_{\bsxi}$ at $x$ is therefore the mass at $x$,
\[
F_{\bsxi}(x) - F_{\bsxi}(x^-) = P[\bsxi = x],
\]
and $F_{\bsxi}$ is continuous {\red exactly when $\bsxi$ has no atom}.
\begin{itemize}
\item
{\blue Discrete}: $F_{\bsxi}(x) = \sum_{k \,:\, \xi_k \leq x} p_k$ increases only by jumps, of height $p_k$ at each $\xi_k$. With finitely many scenarios, it is a {\red step function}: right-continuous, but not continuous.
\item
{\blue With a density}: there is no atom, so $F_{\bsxi}$ is continuous. The converse fails --- a continuous cdf need not have a density --- but the course never meets that case.
\end{itemize}
...
\]
defined whether or not $F_{\bsxi}$ is invertible --- in particular when $F_{\bsxi}$ has jumps.

\smallskip

{\blue Two uses in the course.}
\begin{itemize}
\item
{\red Inversion sampling} (deck~02). For $u \in (0,1)$, $F_{\bsxi}^{-1}(u) \leq x \iff u \leq F_{\bsxi}(x)$, the infimum being attained by right-continuity, so $P[F_{\bsxi}^{-1}(\bsu) \leq x] = F_{\bsxi}(x)$ for $\bsu \sim U[0,1]$ --- {\red for every} $F_{\bsxi}$, discrete ones included.

Conversely, if $\bsxi$ has no atom, $F_{\bsxi}(\bsxi) \sim U[0,1]$: $F_{\bsxi}$ is continuous, so $F_{\bsxi}(F_{\bsxi}^{-1}(u)) = u$ (right-continuity gives $\geq$, left-continuity $\leq$). $F_{\bsxi}$ may still not be invertible, being constant across any gap of the support, but $\lbrace F_{\bsxi}(\bsxi) \leq u \rbrace$ and $\lbrace \bsxi \leq F_{\bsxi}^{-1}(u) \rbrace$ differ only on such a flat stretch, of probability zero:
\[
P[F_{\bsxi}(\bsxi) \leq u] = P\left[\bsxi \leq F_{\bsxi}^{-1}(u)\right] = F_{\bsxi}\left(F_{\bsxi}^{-1}(u)\right) = u, \qquad u \in (0,1).
\]
\item
A chance constraint $P[\cdot] \geq \alpha$ is a statement about a quantile (deck~05).
\end{itemize}
...
\begin{center}
\begin{tikzpicture}[x=1cm,y=0.86cm,>=latex,font=\footnotesize]
  % axes
  ...
  % support: the atom, plus the closure of the region where F increases --- and
  % NOT the flat stretch in between, which the variable never visits
  \fill[teal] (1.4,-0.95) circle (2.2pt);
  \draw[decorate,decoration={brace,mirror,amplitude=4pt},teal,line width=0.7pt] (2.6,-0.95) -- (5.0,-0.95);
  \node[teal,below,font=\scriptsize] at (3.3,-1.12) {support $\Xi$ --- the gap belongs to neither piece};
  \node[align=left,font=\scriptsize,anchor=west] at (3.3,0.55)
     {jump at $1.4$: an atom,\\$P[\bsxi = 1.4] > 0$};
\end{tikzpicture}
\end{center}

{\footnotesize A flat stretch of $F$ is a region the variable never visits; a jump is an atom. The quantile is read horizontally then downwards, the $\inf$ picking the left end of a flat stretch. The support is what is left, closed up --- here {\red disconnected}.}
...
For an integer $p \geq 1$, the {\red moment of order $p$} of a random variable $\bsxi$ is $\EE[\bsxi^p]$, and its {\red central moment} of order $p$ is $\EE\left[(\bsxi - \EE[\bsxi])^p\right]$: the mean is the first moment, the variance the second central moment.
For any real $p > 0$, the {\red absolute moment} of order $p$ is $\EE[|\bsxi|^p]$.

\mbox{}

$\bsxi$ has {\red finite moments of order $p$} when $\EE[|\bsxi|^p] < \infty$, which makes $\EE[\bsxi^p]$ well defined and finite; for $p = 1$, $\bsxi$ is said to be {\red integrable}.
A random vector has finite moments of order $p$ when $\EE[\|\bsxi\|^p] < \infty$.
For $p = 2$, as $\|\bsxi\|^2 = \sum_i \bsxi_i^2$, this asks every component to have a finite second moment: it is the hypothesis {\red ``$\bsxi$ has finite second-order moments''}, which appears eight times in deck~03 alone.

\mbox{}

{\blue Lower orders come for free.} If $0 < r \leq p$, then $|x|^r \leq 1 + |x|^p$ for every $x$ (bound $|x|^r$ by $1$ if $|x| \leq 1$, by $|x|^p$ otherwise), hence
\[
\EE\left[|\bsxi|^r\right] \leq 1 + \EE\left[|\bsxi|^p\right].
\]
In particular, finite second-order moments give $\EE[|\bsxi|] < \infty$: the mean exists and is finite.
The converse fails: with $P[\bsxi = k] = c/k^3$, $k = 1, 2, \ldots$, the mean $c\sum_k 1/k^2$ is finite, but $\EE[\bsxi^2] = c\sum_k 1/k = +\infty$.
...
The {\red variance} is the second central moment,
\[
\var[\bsxi] = \EE\left[(\bsxi - \EE[\bsxi])^2\right] = \EE[\bsxi^2] - \EE[\bsxi]^2.
\]
It presupposes a finite mean, which finite second-order moments guarantee (previous slide); expanding the square then gives the second form.
Conversely, $\EE[\bsxi^2] = \var[\bsxi] + \EE[\bsxi]^2$: a random vector has finite second-order moments {\red if and only if} every component has a finite variance.

\smallskip

For two such variables, $\cov[\bsxi, \bseta] = \EE\left[(\bsxi - \EE[\bsxi])(\bseta - \EE[\bseta])\right]$ is finite too, by Cauchy--Schwarz below; the covariance matrix $\Sigma = \EE\left[(\bsxi-\EE[\bsxi])(\bsxi-\EE[\bsxi])^T\right]$ of a random vector is symmetric positive semi-definite.
...
\]
In deck~03 the recourse cost is $q(\bsxi)^Tz(\bsxi)$, $z = h - Tx$, and $\bsxi$ gathers the random entries of $q$, $h$, $T$. Finite second-order moments therefore make $\EE$ of that product finite --- which is precisely what makes $\calQ(x)$ finite, and what the simple-recourse proof there invokes.
...
{\footnotesize With $\bsxi$ taking two values, $\EE[\varphi(\bsxi)]$ sits on the chord and $\varphi(\EE[\bsxi])$ on the curve, below it. Deck~03 obtains $\EV \leq \WS$ this way: the value function of a linear program is convex in its right-hand side, so replacing the latter by its mean can only look {\it too good} --- as does using $\EE[\bsxi]$, if $h$ is affine.}
...
Expectation & $\EE[\cdot]$, $\EE_{\bsxi}[\cdot]$ \\
